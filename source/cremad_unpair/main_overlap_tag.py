...
def eval_overlap_tag(loader, device, args):
    ...
    tea_model.load_state_dict(
        torch.load('/ckpts/cremad_ckpts/teacher_mod_' + str(tea_type) + '_' + arch + '_' + str(args.num_frame) + '_overlap.pkl',
    ...
    if args.method_type == "bilevel":
        kernel_type = args.kappa_type
        acc = train_network_distill_unpair_bilevel(stu_type, tea_model, args.num_epochs, loader, net, device, optimizer, warmup_lr_scheduler, main_lr_scheduler, lr_scheduler, args, tea, stu, kappa_type= kernel_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--kappa_type", type=float, default= 1000)
    ...
    loader = gen_data('/CREMA-D', args.batch_size, args.num_workers, args)
    if args.pre_train:
        loader_fb = gen_data('/CREMA-D', args.batch_size2, args.num_workers, args)
